...
from Uni_OPD_utils.outcome_reward.manager.server import ServerManager  # noqa
...
def llm_as_judge_sync(predict_str, ground_truth, extra_info):
    ...
    if DEBUG:
        logger.info(f"LLM Judge prompt: {prompt}")
    ...
    response: str = server_manager.call(question=prompt)
    if DEBUG:
...
def compute_score(
    ...
    """
    use_llm, llm_time = False, 0.0

    if use_format_reward:
        format_score_ratio = 0.1
        format_score = format_reward(solution)
    else:
        format_score_ratio = 0.0
        format_score = 0.0

    gt: str = simple_parse(ground_truth)

    extracted_answer = extract_anwser_tag(solution).strip()
    predict_str: str = simple_parse(extracted_answer)

    if DEBUG:
        logger.info(f"Extracted answer: {predict_str}")
    ...
    if predict_str != "":
        ...
        if acc_score == 0.0 and _is_True(USE_LLM_JUDGE):
            use_llm, start_time = True, time.time()
    ...
    if (
        acc_score == 0.0
        and _is_True(USE_LLM_JUDGE)
        and ((not use_format_reward) or format_score == 0.0)
        and len(solution) < 500
    ):
        if DEBUG:
            logger.info("Call LLM again")
        # Direct judge the solution string with the ground truth
        use_llm, start_time = True, time.time()
        acc_score = llm_as_judge_sync(solution, ground_truth, extra_info)
        llm_time += time.time() - start_time

    score: float = (1.0 - format_score_ratio) * acc_score + format_score_ratio * format_score
