...
def _load_module_from_string(name: str, source: str):
    """用标准库替代 RuntimeModule.from_string."""
...
def run_test(in_outs, test=None, debug=False, timeout=15):
    ...
    in_outs: 包含 inputs/outputs 和可选 fn_name 的字典
    test: 待测试的 Python 代码字符串
    debug: 是否打印调试信息
    timeout: 超时时间
    """
    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                tmp_sol = _load_module_from_string("tmp_sol", sol)
...
"""
    ...
    res, metadata = run_test(in_outs=test_cases, test=code, debug=True, timeout=5)
    print(f"res = {res}, metadata = {metadata}")
