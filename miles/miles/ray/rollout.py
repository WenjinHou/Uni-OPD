...
from miles.ray.rollout_logs import log_eval_rollout, log_rollout
...
class RolloutManager:
    ...
    def generate(self, rollout_id: int) -> list[Box]:
        start_time = time.time()
        self.rollout_id = rollout_id
        # 1. 开始时恢复 RolloutHealthMonitor 后台线程，使其开始监测 rollout engine 是否存活
        ...
        # 3. 调用 generate_rollout，通过 SGLang engine 做推理生成 rollout 数据，同时打分
        data, metrics = self._get_rollout_data(rollout_id=rollout_id)
        self._save_debug_rollout_data(data, rollout_id=rollout_id, evaluation=False)

        rollout_time: float = time.time() - start_time  # 整个 rollout 过程的耗时
        log_rollout(rollout_id, self.args, data, metrics, rollout_time)

        # 4. 将 Sample 对象列表转为训练用的 dict
        data = self._convert_samples_to_train_data(data)

        # 5. 按 DP 切分返回
        boxes: list[Box] = self._split_train_data_by_dp(data, self.train_parallel_config["dp_size"])
        return boxes
    ...
    def eval(self, rollout_id):
        ...
        metrics = log_eval_rollout(rollout_id, self.args, data, result.metrics)
        if self._metric_checker is not None:
    ...
    def _get_rollout_data(self, rollout_id) -> tuple[list[Sample], dict | None]:
        # DEBUG: 从磁盘加载 .pt 文件，跳过实际推理
        if self.args.load_debug_rollout_data:
            ...
            if self.use_experimental_refactor:  # 不设置环境变量就是 False
                data: RolloutFnTrainOutput = call_rollout_function(
                    self.generate_rollout,
                    RolloutFnTrainInput(rollout_id=rollout_id),
                )
            else:
                # 推理
                data: RolloutFnTrainOutput = call_rollout_fn(
                    self.generate_rollout,  # 默认为 miles.rollout.sglang_rollout.generate_rollout
                    self.args,
                    rollout_id,
                    self.data_source,
                    evaluation=False,
            ...
            data: list[list[Sample]] = data.samples
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample]) -> dict:
        ...
        train_data = {
            ...
            "sample_group_index": [sample.group_index for sample in samples],
        }

        # loss mask
        # TODO: compress the loss mask
        loss_masks, removed_cnt = [], 0
        for sample in samples:
            ...
            # 被 Rollout Sample Filter 处理后被过滤掉的样本
            if sample.remove_sample:
                sample.loss_mask = [0] * sample.response_length
                removed_cnt += 1
            loss_masks.append(sample.loss_mask)
        if removed_cnt > 0:
            logger.info(f"[Filter] Removed {removed_cnt} samples from rollout data")
        ...
        if self.args.advantage_estimator == "on_policy_distillation":
            if "teacher_log_probs" in samples[0].__dict__:
                ...
                print("No teacher_log_probs found in samples, skipping.")
            if "response_correct" in samples[0].__dict__:
