...
def _collect_correctness_advantages(
    rollout_data: RolloutBatch,
    log_dict: dict[str, float],
    parallel_state: ParallelState,
) -> None:
    response_correct = rollout_data.get("response_correct", None)
    advantages_list = rollout_data.get("advantages", None)
    if response_correct is None or advantages_list is None or not any(rc is not None for rc in response_correct):
        return

    correct_adv_sum = 0.0
    correct_adv_count = 0
    incorrect_adv_sum = 0.0
    incorrect_adv_count = 0
    for i, rc in enumerate(response_correct):
        if rc is None:
            continue
        adv = advantages_list[i]
        if isinstance(adv, torch.Tensor) and torch.all(adv == 0):
            continue
        sample_mean = adv.mean().item() if isinstance(adv, torch.Tensor) else float(adv)
        if rc is True:
            correct_adv_sum += sample_mean
            correct_adv_count += 1
        elif rc is False:
            incorrect_adv_sum += sample_mean
            incorrect_adv_count += 1

    stats = torch.tensor(
        [correct_adv_sum, correct_adv_count, incorrect_adv_sum, incorrect_adv_count],
        dtype=torch.float64,
        device="cuda",
    )
    dist.all_reduce(stats, op=dist.ReduceOp.SUM, group=parallel_state.dp_cp_group)
    global_correct_sum, global_correct_count, global_incorrect_sum, global_incorrect_count = stats.tolist()

    correct_mean = global_correct_sum / global_correct_count if global_correct_count > 0 else None
    incorrect_mean = global_incorrect_sum / global_incorrect_count if global_incorrect_count > 0 else None

    if correct_mean is not None:
        log_dict["advantages/correct"] = correct_mean
    if incorrect_mean is not None:
        log_dict["advantages/incorrect"] = incorrect_mean
    if correct_mean is not None and incorrect_mean is not None:
        log_dict["advantages/correct_minus_incorrect"] = correct_mean - incorrect_mean
...
def log_rollout_data(
    ...
    if parallel_state.tp_rank == 0 and parallel_state.is_pp_last_stage:
        ...
        # 按正确性分组统计 advantages（通过 allreduce 在 gather 前完成全局聚合）
        _collect_correctness_advantages(rollout_data, log_dict, parallel_state)
