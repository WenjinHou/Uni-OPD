...
from Uni_OPD_utils.margin_calibration.margin_mask_greedy import opd_correctness_margin_mask
from Uni_OPD_utils.margin_calibration.margin_shift import opd_correctness_margin_shift
...
def compute_advantages_and_returns(args: Namespace, parallel_state: ParallelState, rollout_data: RolloutBatch) -> None:
    ...
    #! OPD
        ...
        student_log_probs: list[torch.Tensor] = log_probs  # len: batch_size, shape: (response_length)
        teacher_log_probs: list[torch.Tensor] = rollout_data["teacher_log_probs"]
        response_lengths = rollout_data["response_lengths"]  # len: batch_size
        ...
        # 构建哨兵 mask：失败样本的 teacher token 位置 advantage 置 0，不影响梯度
        advantages: list[torch.Tensor] = []
        for i, (t_logps, s_logps) in enumerate(zip(teacher_log_probs, student_log_probs, strict=False)):
            if t_logps.shape[0] != s_logps.shape[0]:
                logger.warning(
                    f"[OPD] teacher/student log_prob length mismatch at sample {i}: "
                    f"teacher={t_logps.shape[0]}, student={s_logps.shape[0]}. "
                )
                adv = torch.zeros_like(s_logps)
            else:
                teacher_valid_mask = (t_logps != TEACHER_LOGP_FAILED_SENTINEL).float()
                adv: torch.Tensor = (t_logps - s_logps) * teacher_valid_mask  # 失败位置直接置 0
            advantages.append(adv)

        # 基于 response 正确性调整 advantage，使正确样本的 advantage 高于错误样本
        if args.use_opd_margin_shift is True:
            advantages: list[torch.Tensor] = opd_correctness_margin_shift(
                args=args,
        ...
        # 基于 response 正确性执行贪婪优势掩码，剔除不满足 margin 的样本
        if args.use_opd_margin_mask_greedy is True:
            advantages: list[torch.Tensor] = opd_correctness_margin_mask(
                args=args,
                advantages=advantages,
                rollout_data=rollout_data,
                parallel_state=parallel_state,
    ...
    if args.normalize_advantages:  # 默认为 False
...
def _build_teacher_failed_mask(teacher_log_probs_cat: torch.Tensor) -> torch.Tensor:
    """检测 teacher_log_probs 中的哨兵值 (-100)，返回有效位置的 float mask。

    哨兵位置为 0（mask 掉），正常位置为 1。
    """
    return (teacher_log_probs_cat != TEACHER_LOGP_FAILED_SENTINEL).float()


# 被 miles/backends/megatron_utils/model.py 的 train_one_step 方法调用
def policy_loss_function(
    ...
    if args.advantage_estimator == "on_policy_distillation":
        teacher_log_probs_cat = torch.cat(batch["teacher_log_probs"], dim=0)
        # ── 哨兵 mask：失败的 teacher token 直接置零，不参与 clip 计算也不参与梯度 ──
        teacher_valid_mask = _build_teacher_failed_mask(teacher_log_probs_cat)
        num_failed_tokens = (teacher_valid_mask == 0).sum().item()
        if num_failed_tokens > 0:
            logger.warning(
                f"[policy_loss_function] {num_failed_tokens}/{teacher_valid_mask.numel()} "
                f"teacher tokens are sentinel (-100), zeroing their pg_loss."
            )
            failed_pg_loss = pg_loss[teacher_valid_mask == 0]
            num_nonzero_failed = (failed_pg_loss != 0).sum().item()
            if num_nonzero_failed > 0:
                logger.error(
                    f"[policy_loss_function] BUG DETECTED: {num_nonzero_failed}/{num_failed_tokens} "
                    f"failed teacher tokens have non-zero pg_loss (expected 0 from advantage masking). "
                    f"pg_loss stats on failed tokens: "
                    f"max={failed_pg_loss.abs().max().item():.6f}, "
                    f"mean={failed_pg_loss.abs().mean().item():.6f}. "
                    f"Applying teacher_valid_mask as fallback."
                )
                pg_loss = pg_loss * teacher_valid_mask
            else:
                logger.info("No need to apply teacher_valid_mask, all failed tokens have zero pg_loss.")

        # Apply teacher-student logprob clip mask directly on pg_loss (without modifying batch["loss_masks"])
        if args.use_teacher_student_logprob_clip:
            teacher_student_clip_range = args.teacher_student_logprob_clip_range
            logprob_k1 = compute_approx_kl(log_probs, teacher_log_probs_cat, "kl")
            # clip mask 仅对有效 token 生效，失败 token 直接为 0
            opd_clip_mask = (logprob_k1.abs() < teacher_student_clip_range).float()

            pg_loss_zeros_before = (pg_loss == 0).float().sum()
            pg_loss_total = pg_loss.numel()
            logger.info(
                f"[policy_loss_function] pg_loss zeros BEFORE teacher-student mask: "
                f"{pg_loss_zeros_before.item():.0f} / {pg_loss_total} "
                f"({100 * pg_loss_zeros_before.item() / pg_loss_total:.2f}%)"
            )

            pg_loss = pg_loss * opd_clip_mask

            # Debug: pg_loss zeros after mask
            pg_loss_zeros_after = (pg_loss == 0).float().sum()
            logger.info(
                f"[policy_loss_function] pg_loss zeros AFTER teacher-student mask: "
                f"{pg_loss_zeros_after.item():.0f} / {pg_loss_total} "
                f"({100 * pg_loss_zeros_after.item() / pg_loss_total:.2f}%)"
            )
            logger.info(
                f"[policy_loss_function] opd_clip_mask.sum: {opd_clip_mask.sum().item():.0f}, "
                f"newly masked by opd_clip+sentinel: "
                f"{(pg_loss_zeros_after - pg_loss_zeros_before).item():.0f} "
                f"({100 * (pg_loss_zeros_after - pg_loss_zeros_before).item() / pg_loss_total:.2f}%)"
            )
