...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    sample.rollout_time = time.time() - start_time
    return sample
...
async def generate_and_rm(
    ...
    if args.advantage_estimator == "on_policy_distillation" and sample.teacher_model_name is None:
        logger.warning(f"111 sample.teacher_model_name is None, sample: {sample}")
    ...
    if isinstance(sample, list):
        ...
        for sample, reward in zip(samples_need_reward, rewards, strict=False):
            ...
            if (
                ...
                and "reward_time" in reward
                and reward["reward_time"]
                and sample.reward_time == 0
            ):
                sample.reward_time = reward["reward_time"]
        return samples
    else:
        ...
        if sample.reward is None:
            if args.advantage_estimator == "on_policy_distillation" and sample.teacher_model_name is None:
                logger.warning(f"222 sample.teacher_model_name is None, sample: {sample}")
            sample.reward = await async_rm(args, sample)

        if (
            isinstance(sample.reward, dict)
            and "reward_time" in sample.reward
            and sample.reward["reward_time"]
            and sample.reward_time == 0
        ):
            sample.reward_time = sample.reward["reward_time"]
    return sample
...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        ...
        for task in done:
            group: list[Sample] = task.result()  # 一个 group = n_samples_per_prompt 个 Sample

            if do_print:
                sample: Sample = group[0][0] if isinstance(group[0], list) else group[0]
                if args.advantage_estimator == "on_policy_distillation":
                    # sample.reward 是一个 dict，pop 掉 meta_info 这个键值对
                    reward_str = {k: v for k, v in sample.reward.items() if k != "meta_info"}
                else:
                    reward_str = sample.reward
                text = str(sample.prompt) + sample.response
                logger.info(f"First rollout sample: {[text]}, label: {str(sample.label)[:100]}, reward: {reward_str}")
                do_print = False
    ...
    pbar.close()
    sample: Sample = data[-1][0][0] if isinstance(data[-1][0], list) else data[-1][0]
    if args.advantage_estimator == "on_policy_distillation":
        # sample.reward 是一个 dict，pop 掉 meta_info 这个键值对
        reward_str = {k: v for k, v in sample.reward.items() if k != "meta_info"}
    else:
        reward_str = sample.reward
    text = str(sample.prompt) + sample.response
    logger.info(f"Finish rollout: {[text]}, label: {str(sample.label)[:100]}, reward: {reward_str}")
