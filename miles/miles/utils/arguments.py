    ...
    def add_miles_arguments(parser):
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Whether to adjust OPD advantages based on response correctness via margin shifting.",
            )
            parser.add_argument(
                "--opd-margin-scope",
                type=str,
                ...
                help="Scope for computing correctness-based advantage statistics. "
                "'global': use all samples in the batch; "
                "'group': use samples within each prompt group (requires sample_group_index).",
            )
            parser.add_argument(
                "--opd-margin-mode",
                type=str,
                default="mean",
                choices=["mean", "minmax"],
                help="Mode for margin shifting. "
                "'mean': require mean(correct_adv) - mean(incorrect_adv) > margin; "
                "'minmax': require min(correct_adv) - max(incorrect_adv) > margin.",
            )
            parser.add_argument(
                "--opd-margin-delta",
                type=float,
                default=0.0,
                help="The margin delta for correctness-based advantage shifting.",
            )
            parser.add_argument(
                ...
                help="Direction of margin shifting. "
                "'correct_up': shift correct samples up by the full margin; "
                "'incorrect_down': shift incorrect samples down by the full margin; "
                "'both': shift correct samples up and incorrect samples down by half the margin each.",
            )
            # ---- Greedy Margin Mask ----
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Whether to use greedy advantage masking to iteratively remove samples "
                "that violate the margin condition. Removed samples have their advantages zeroed out. "
                "Reuses --opd-margin-scope, --opd-margin-mode, "
                "--opd-margin-delta for scope/mode/delta configuration.",
            )
            parser.add_argument(
                "--opd-mask-min-retain-ratio",
                type=float,
                default=0.25,
                help="Minimum fraction of correct/incorrect samples to retain during greedy masking. "
                "Prevents excessive removal that could cause gradient vanishing. Range: (0, 1].",
            )
            parser.add_argument(
                "--use-teacher-student-logprob-clip",
                action="store_true",
                default=False,
                help="whether to use teacher student logprob clip",
            )
            # teacher_student_logprob_clip_range
            parser.add_argument(
                "--teacher-student-logprob-clip-range",
                type=float,
                default=10.0,
                help="The clipping range for teacher student logprob clip.",
            )
            return parser
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                ...
                action=argparse.BooleanOptionalAction,
                default=True,
                help="Whether to use format reward in addition to the main reward.",
            )
            parser.add_argument(
                "--eval-reward-key",
                type=str,
        ...
        def add_rollout_buffer_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Enable filtering rollout samples based on response_correct to balance correct/incorrect ratio.",
            )
            parser.add_argument(
                "--balance-rollout-scope",
                type=str,
                default="global",
                choices=["global", "sample"],
                help=(
                    "Mode for filtering rollout samples. "
                    "'global': filter across all samples globally; "
                    "'sample': filter within each prompt group (samples sharing the same group_index)."
                ),
            )
            parser.add_argument(
                "--balance-rollout-ratio",
                type=float,
                default=1.0,
                help=(
                    "Target ratio of correct samples to incorrect samples after filtering. "
                    "E.g., 1.0 means 1:1 correct:incorrect ratio. "
                    "Excess samples of the majority class will be masked (remove_sample=True)."
                ),
            )
            parser.add_argument(
