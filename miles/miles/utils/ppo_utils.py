...
def compute_policy_loss(
    ...
    eps_clip: float,  # PPO clip range
    eps_clip_high: float,  # PPO clip upper range
    eps_clip_c: float | None = None,  # Lower bound for Dual-clip PPO
):
    ratio = (-ppo_kl).exp()
    pg_losses1 = -ratio * advantages
    # pg_losses2: - clip(ratio, 1-cliprange, 1+cliprange) * A
    pg_losses2 = -ratio.clamp(1 - eps_clip, 1 + eps_clip_high) * advantages
    # clip_pg_losses1: max(-ratio * A, -clip(ratio, 1-cliprange, 1+cliprange) * A)
