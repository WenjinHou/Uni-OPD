...
class Sample:
    ...
    group_index: int | None = None  # 组索引（prompt 级别），同一个 prompt 下的所有 sample 共享
    index: int | None = None  # 样本索引（全局唯一）
    #! Prompt
    prompt: str | list[dict[str, str]] = ""
    tokens: list[int] = field(default_factory=list)  # 在 sglang_rollout.py 中会初始化 sample.tokens = prompt_ids
    multimodal_inputs: dict[str, Any] = None  # raw multimodal data, e.g. images, videos, etc.
    #  {"images": [PIL.Image.Image], "videos": None}
    multimodal_train_inputs: dict[str, Any] = None  # processed multimodal data, e.g. pixel_values, etc.

    #! Response
    response: str = ""
    response_length: int = 0
    label: str | None = None  # Ground truth answer
    reward: float | dict[str, Any] | None = None
    loss_mask: list[int] | None = None
    weight_versions: list[str] = field(default_factory=list)
    rollout_log_probs: list[float] | None = None  # Log probabilities from rollout engine
    teacher_log_probs: list[float] | None = None  # Log probabilities from teacher model
    ...
    teacher_model_name: str | None = None  # the teacher model used for OPD
    ...
    def get_reward_value(self, args: Namespace) -> int | float:
        if isinstance(self.reward, (int, float)):
            return self.reward
